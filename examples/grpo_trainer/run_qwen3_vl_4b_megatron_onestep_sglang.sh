#!/usr/bin/env bash
# ==============================================================================
# verl One-Step Off-Policy GRPO | Qwen3-VL-4B | SGLang rollout | Megatron
# 4× GPU 分卡同步: 训练 2 卡 (TP2) + 推理 2 卡 (TP2) — 串行执行
#
# 参数 1:1 对标 slime: scripts/run-qwen3-VL-4B-geo3k-4gpu-v3.sh
#
# 与 slime 架构完全一致:
#   1. 训练和推理使用独立 GPU 池 (2+2)
#   2. 串行执行: rollout → log_prob → train → update_weights → repeat
#   3. NCCL CheckpointEngine 做权重同步
#   4. hybrid_engine=False (分离模式)
#
# 与 fully_async 的关键区别:
#   - 同步而非异步 — rollouter 和 trainer 按顺序执行
#   - 无 MessageQueue / staleness 控制
#   - 入口: verl.experimental.one_step_off_policy.main_ppo
# ==============================================================================
...
SAVE_DIR="${SAVE_DIR:-/workspace/volume/pengxiong/models/Qwen3-VL-4B_verl_onestep_sglang_v1}"
...
experiment_name=${EXPERIMENT_NAME:-qwen3_vl_4b_sglang_megatron_onestep_v1}
# ---- end user-adjustable ----
...
echo "============================================================"
echo "verl One-Step Off-Policy GRPO | Qwen3-VL-4B | SGLang + Megatron"
