#!/usr/bin/env bash
# Performance Test (Fully Async): Qwen3-4B | GRPO | Megatron + SGLang | 8×A100 40GB
#
# SGLang variant — identical to run_qwen3_4b_megatron_perf_test_async.sh
# except rollout engine is SGLang instead of vLLM.
#
# GPU allocation: 4 train (TP2, DP2) + 4 inference (2 engines × TP2)
# Matches slime: scripts/run-qwen3-4B-async-8gpu.sh (SGLang rollout)
#
# Usage:
#   bash examples/grpo_trainer/run_qwen3_4b_megatron_perf_test_async_sglang.sh
#
# Override env vars:
...
EXPERIMENT_NAME=${EXPERIMENT_NAME:-qwen3_4b_grpo_n16_resp8192_megatron_async_sglang_4tp2_4tp2}
