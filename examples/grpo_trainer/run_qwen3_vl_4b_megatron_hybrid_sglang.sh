#!/usr/bin/env bash
# ==============================================================================
# verl Hybrid Engine GRPO | Qwen3-VL-4B | SGLang rollout | Megatron | 4× GPU
#
# 参数 1:1 对标 slime: scripts/run-qwen3-VL-4B-geo3k-4gpu-v3.sh
#
# GPU: 4× hybrid engine — 训练和推理共享 4 卡 (3D-HybridEngine resharding)
#   slime:     2 GPU 训练 (TP2) + 2 GPU 推理 (1×SGLang TP2), 串行
#   verl 等价: Megatron TP2 + SGLang TP2, 4 GPU hybrid engine colocate
#
# 与 slime 完全对齐的参数:
#   --n-samples-per-prompt 8      → rollout.n=8
#   --global-batch-size 64        → ppo_mini_batch_size=64
#   --rollout-max-response-len 3072 → max_response_length=3072
#   --lr 1e-6                     → optim.lr=1e-6
#   --eps-clip 0.2 --eps-clip-high 0.28
#   --tensor-model-parallel-size 2 → megatron.tp=2
#   --rollout-num-gpus-per-engine 2 → rollout.tp=2
#   --sglang-mem-fraction-static 0.7 → gpu_memory_utilization=0.7
#   --sglang-mm-attention-backend sdpa
#   --sglang-disable-cuda-graph (→ enforce_eager=True)
# ==============================================================================
...
SAVE_DIR="${SAVE_DIR:-/workspace/volume/pengxiong/models/Qwen3-VL-4B_verl_hybrid_sglang_v1}"
...
total_training_steps=${TOTAL_TRAINING_STEPS:-12}       # 性能测试用, slime 生产用 500
...
experiment_name=${EXPERIMENT_NAME:-qwen3_vl_4b_sglang_megatron_hybrid_v1}
# ---- end user-adjustable ----
...
echo "============================================================"
echo "verl Hybrid Engine GRPO | Qwen3-VL-4B | SGLang + Megatron"
